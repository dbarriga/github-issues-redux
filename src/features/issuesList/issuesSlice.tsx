...
import {IssuesResult, Issue, getIssue, getIssues} from '../../api/githubAPI';
...
const issues = createSlice({
  ...
  reducers: {
    ...
    getIssueSuccess(state: IssuesState, {payload}: PayloadAction<Issue>){
      const {number} = payload;
      state.issuesByNumber[number] = payload;
      state.isLoading = false;
      state.error = null;
    },
    getIssuesSuccess(state: IssuesState, action: PayloadAction<IssuesResult>) {
...
export const {getIssueStart, getIssuesStart, getIssueSuccess, getIssuesSuccess, getIssueFailure, getIssuesFailure} = issues.actions;
